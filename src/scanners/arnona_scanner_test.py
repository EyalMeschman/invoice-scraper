...
async def download_invoice_by_period(
    ...
    row = page.locator(f"table#datatable tbody tr:has-text('{period.value}')").first
    ...
    save_path = download_dir / f"{period.value}.pdf"
...
async def test_arnona(
    ...
    download_dir = Path(f"downloads/{YEAR}/arnona")
