...
class Utils:
    ...
    @staticmethod
    async def wait_for_authenticated_page(page: Page, url: str, platform: str):
        try:
    ...
    async def wait_for_authenticated_selector(
        page: Page, selector: str, should_exist: bool, domain: str
    ):
        try:
            await page.wait_for_selector(selector)
            if not should_exist:
                raise CookieExpiredError(domain)
        except PlaywrightTimeoutError as exc:
            if not should_exist:
                return
            raise CookieExpiredError(domain) from exc
    ...
    async def download_pdf_from_blob_url(page: Page, blob_url: str) -> bytes:
        """
        Download PDF content from a blob URL using JavaScript evaluation.

        Args:
            page: The Playwright page context from which to fetch the blob
            blob_url: The blob URL to download from
